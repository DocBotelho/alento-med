class Syncer
  ...
  def sync
    ...
    Study.joins("INNER JOIN facilities ON facilities.nct_id = studies.nct_id INNER JOIN links ON links.nct_id = studies.nct_id").where(WHERE).select("links.nct_id as links_nct_id, links.url as url").each do |url|
      ...
      # if trial.description == nil
      #   trial.description = desc.intervention_description if trial.trial_nct_id == desc.interventions_nct_id
      # end
      # if trial.description == nil
      #   trial.description = desc.design_groups_description if trial.trial_nct_id == desc.design_groups_nct_id
