class Syncer
  ...
  Study.joins("INNER JOIN facilities on facilities.nct_id = studies.nct_id").where(WHERE, FACILITYWHERE).select("facilities.id as facility_id, facilities.nct_id as facility_nct_id, facilities.name as facility_name, facilities.city as facility_city, facilities.state as facility_state, facilities.zip as facility_zip, facilities.country as facility_country").each do |facility|

    institution = Institution.find_or_initialize_by(facility_id: facility.facility_id) if trial = Trial.find_by(trial_nct_id: facility.facility_nct_id)
    ...
    if institution.persisted?
        institution.institution_nct_id += facility.facility_nct_id + " " unless institution.institution_nct_id.include?(facility.facility_nct_id)
    else
      institution.institution_nct_id =  facility.facility_nct_id
    end
    institution.save
    ...
    Study.joins("INNER JOIN facilities on facilities.nct_id = studies.nct_id INNER JOIN facility_contacts on facility_contacts.facility_id = facilities.id").where(WHERE, FACILITYWHERE).select("facility_contacts.name as facility_contacts_name, facility_contacts.email as facility_contacts_email, facility_contacts.phone as facility_contacts_phone, facility_contacts.facility_id as facility_contacts_facility_id, facility_contacts.nct_id as facility_contacts_nct_id").each do |fcontact|
      institution = Institution.find_by(facility_id: fcontact.facility_contacts_facility_id, institution_nct_id: fcontact.facility_contacts_nct_id)
