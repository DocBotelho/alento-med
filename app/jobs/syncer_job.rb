  ...
  def perform
    ...
    Study.joins("INNER JOIN facilities ON facilities.nct_id = studies.nct_id LEFT OUTER JOIN central_contacts ON central_contacts.nct_id = studies.nct_id INNER JOIN browse_conditions ON browse_conditions.nct_id = studies.nct_id INNER JOIN eligibilities ON eligibilities.nct_id = studies.nct_id LEFT OUTER JOIN design_groups on design_groups.nct_id = studies.nct_id").where("facilities.country = 'Brazil' AND facilities.status = 'Recruiting' AND study_type = 'Interventional'").select("studies.nct_id as study_nct_id, browse_conditions.mesh_term as condition_name, studies.brief_title as brief_title, eligibilities.gender as gender, eligibilities.maximum_age as maximum_age, eligibilities.minimum_age as minimum_age, facilities.name as facility_name, facilities.nct_id as institution_nct_id, facilities.city as facility_city, facilities.state as facility_state, facilities.country as facility_country, facilities.zip as facility_zip, central_contacts.nct_id as contact_nct_id, central_contacts.name as contact_name, central_contacts.email as contact_email, central_contacts.phone as contact_phone, design_groups.description as brief_description").each do |f|
      ...
      institution = Institution.find_or_initialize_by(institution_nct_id: f.institution_nct_id, name: f.facility_name)
      unless institution.persisted?
